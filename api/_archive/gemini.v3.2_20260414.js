 ...
 * 
 * 📝 特點描述：重新整理區塊職責，補一些缺漏
 * [V] 讓AI把思考過程寫下來（模仿thinking，因為 flash-lite 沒有思考過程）
 * [V] 好好聊天（避免變成高高在上，不對用戶其他地方挑毛病，像是取名簡單或中庸之類的）
 * [V] 防止籠統名字 (解決 單說「哀鳳色」「內衣色」會通過 問題)
 * [V] 防止私人經驗名字  (解決「我媽的拖鞋」會通過 問題)
 * [V] 台灣常識與全球常識 (解決AI以為 台股綠色是漲 問題)
 * [V] 重新整理區塊職責（原本可能會放錯地方寫）：
 *     - 個性區： 告訴它「你是誰」（情緒化、想像力豐富、接納度高、邏輯分明）
 *     - 漏斗區： 告訴它「怎麼做判斷」（格式、矛盾、典型色、寬容濾鏡）。第一關定調為「任何單看字就能評斷的邏輯」，第二關才是「跟顏色比對時的寬容邏輯」
 *     - 策略區： 告訴它「開口時要注意什麼」（延續使用者的話題、看情況溫暖或吐槽）
 * [ ] 私人經驗～籠統名字，其實就是太狹義～太廣義，並定義什麼是好聯想（包含迷因）
 */
...
export default async function handler(req) {
  ...
  try {
    ...
    const systemInstructionText = `
      ...
      4. 須為公眾可查證的事物：拒絕任何需要「通靈才能懂」的詞彙。不能只有當事人才知道真偽（如私人經驗：我阿嬤的沙發、昨天下午的天空、某網紅的水壺），也不能是瞎編的自創詞（知識盲區）。只要一般大眾聽完的反應會是「誰知道啊？」，請視為不符。
    ...
    const schema = {
      ...
      properties: {
        ...
        thought_2_format_and_logic: {
          type: "STRING",
          description: "[內心獨白] 審查漏斗第一關：先單看使用者輸入的詞彙（遮住題目顏色），這是名詞短語還是閒聊句子？有無名字自相矛盾？有無特定典型色？是通靈才能懂嗎？"
        },

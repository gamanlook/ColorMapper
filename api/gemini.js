...
export default async function handler(req) {
  ...
  try {
    ...
    const systemInstructionText = `
      ...
      - 以「台灣人腦袋/生活經驗」想像該詞彙的畫面與進行任務，並全面啟動你的內建常識（包含各國傳統色名、美妝流行色、動漫、迷因、品牌印象與日常事物）
        - 當遇到文化差異時，優先以「台灣在地常識」大於「歐美全球通用常識」為原則（如：「膚色」為裸色；台股為紅漲綠跌）
        - 當遇到網路次文化(荒謬/獵奇/惡搞)時，放下嚴肅的物理邏輯，可視為與現實同等有效的幽默常識
      ...
      請先遮住題目顏色，單看字面評斷與猜顏色（而非先偷看了題目顏色再強辯說該詞彙合理）：
      1. 真的試圖在為顏色命名：整個輸入字串必須 100% 構成一個不可分割的「名詞」或「情境修飾詞+名詞」。若字串內夾雜了任何脫離命名本體的對話、閒聊、無意義亂碼、作答心得或贅字，即視為格式不符。
      2. 名字內部不可自相矛盾：
        - ❌矛盾或邏輯互斥：事物本質與顏色詞互斥（如：用極黑的事物形容純白，如「尼哥白」），或光譜極端對立的雙色硬湊（如：紅綠、黑白），須視為不符。
        - ✅相鄰色的過渡調和：可通過（如：灰白、紫紅、藍白）
      3. 名字須在容易聯想範圍：需要通靈(太狹義）<容易聯想(適中)<籠統模糊(太廣義)
        - ❌需要通靈(太狹義）：私人非公開的經驗，只有當事人才能分辨真偽，並非大眾所知（如「我阿嬤的哀鳳色」、「昨日的天空色」），須視為不符。
        - ❌籠統模糊(太廣義)：沒有一個特定代表色/典型色，有太多色相可能性，說了等於沒說（如單說「哀鳳色」、「外套色」、「彩虹色」無法確定是哪種顏色），須視為不符。
        - ✅容易聯想(可通過)：有符合以下其一，即算是大眾可聯想：
          - 有典型色：提到時某色的聯想一黨獨大（如：蘋果=紅，大便=棕）
          - 有明確色：點出特定且存在的色系，收斂範圍不造成模糊（如「太空灰哀鳳色」、「阿嬤紫外套色」）
          - 次文化共識：動漫/迷因/網路梗（如：原諒色是綠色，漂亮女生的大便是粉紅色）
          - 常理可推斷：字句本身好懂（物理現實/生活經驗）

      【第二關：答案與題目相符】(不符即為 true)
      將該詞彙的大眾印象色，與題目的 OKLCH 數據比對，須與當前數值呈現的顏色相符。但在比對時，請套用以下「寬容濾鏡」：
      1. 廣義色系包容：允許一般人對色彩的籠統認知（允許以廣義的基礎色名，來涵蓋帶有深淺濃淡或相鄰色相的顏色）。例如：用「紅色」稱呼暗紅色，用「藍色」或「綠色」稱呼青色。
      *(色相H參考基準：桃348, 紅27, 橘54, 黃95, 綠141, 青210, 藍255, 紫302)*
      ...
      【🔴不收錄 (isSuspicious: true)】：未通過第一關或第二關
        ...
        - 【描述精準但格式不符】(沒過第一關，但有過第二關)：
          友善點出不收錄的具體緣由（如太長或像造句），接著從他的原話中提煉出一個精簡的「名詞」(加 單引號) 推薦給他
        - 【邏輯矛盾 / 完全不準 / 刻意搗蛋 / 閒聊】(其他沒過的狀況)：
          直接拿對方的話題嗆爛/酸爆對方，比對方還過分
    ...
    const schema = {
      ...
      properties: {
        ...
        thought_2_format_and_logic: {
          type: "STRING",
          description: "[內心獨白] 審查漏斗第一關：先單看使用者輸入的詞彙（遮住題目顏色），這是名詞短語還是閒聊句子？有無名字自相矛盾？是好聯想還是太狹義或廣義？"
        },

...
- `2026/01/28（三）` **🤖思考邏輯更紮實**
  - 原本是散亂的規則、範例讓 AI 去讀（容易跳過中間步驟直接憑感覺判定），現在是有條理的思維鏈（一直試錯被大家鑽漏洞後的成果）
...
- `2026/01/26（一）` **🔒咒語分等級**
  - 作者我調教 AI 的咒語（System Instruction，塑造AI的人格特性與如何思考）跟使用者的輸入（User Prompt）拆開來了
...
- `2026/01/14（三）` **🔒咒語搬家**
  - 將原本位於前端的 AI 咒語（Prompt）與回傳格式（Schema）移往後端伺服器
    - 防止惡意竄改、AI 被拿來用作他途，這樣之後如果我要使用付費版的 AI 才不會被濫用
  - 追加「清理舊資料按鈕」，不然取得資料會越來越肥

- `2026/01/13（二）` **🔒隱藏金鑰**
  - 原本我的 Gemini API Key 在網路上裸奔
    - ~~現在設定了白名單，防止金鑰外洩時被濫用（不然我會收到一坨待繳帳單）~~
    - 改寫架構，不再將 API Key 打包進前端程式碼，所有 AI 請求改由後端代理轉發，防止 Key 在瀏覽器端外洩
    - 發現第二點會跟第一點打架，把第一點取消了（不設定白名單，反正大家也找不到 Key）
  - 代價：感覺變慢了
